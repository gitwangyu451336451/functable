            ...
            <div class='search-input'>
              ...
              <div class="search-input-div"
                ...
                <Select :label-in-value='true'
                        ...
                        :placeholder="colSearchChoosed.placeholder || '请选择'"
                        v-model='searchValue'>
              ...
              <div class="search-input-div"
                ...
                <AutoComplete v-model='searchValue'
                              ...
                              :placeholder='colSearchChoosed.placeholder || "搜索"'>
                </AutoComplete>
              </div>
              <div v-else
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.searchValueList = this.searchValueList.concat(
        this.tableAttr.searchDefaultValueList || []
      )
    ...
    refreshTable(params, isResetPage, wsValue, wsFunc) {
      ...
      if (isResetPage) {
        this.pageData.current = this.tableAttr.pageData
          ? this.tableAttr.pageData.current || 1
          : 1
        this.pageData.pageSize = this.tableAttr.pageData
          ? this.tableAttr.pageData.pageSize || 10
          : 10
      }
      ...
      if (this.tableAttr.source.type === 'staticData') {
        ...
        if (this.tableAttr.isFilter) {
          ...
          this.filterTableData(
            this.baseData,
            this.tableAttr.filterChoosedArr || [],
            this.tableAttr.filterKey
          )
        }
    ...
    compatiblePage(params, isResetPage) {
      if (
        ...
        this.baseData.dataList.length === 0 &&
    ...
    search() {
      ...
      if ((this.searchValue + '').trim() !== '') {
        ...
        if (this.colSearchChoosed.searchType === 'select') {
          obj.name = this.getNameByValue(
            this.searchValue,
            this.colSearchChoosed.searchData
          )
        } else {
        ...
        if (!isOld) {
          // 过滤searchFilterValues的值，比如全部我可以选但是不想加入搜索
          if (
            this.colSearchChoosed.searchType === 'select' &&
            this.colSearchChoosed.searchFilterValues &&
            this.colSearchChoosed.searchFilterValues.length > 0
          ) {
            if (
              this.colSearchChoosed.searchFilterValues.indexOf(obj.value) !== -1
            ) {
              obj.disabledSearch = true
            } else {
              obj.disabledSearch = false
    ...
    // 过滤不显示的行
    filterTableData(baseData, choosedArr, key) {
      // 过滤tableData
      choosedArr.length > 0 &&
        choosedArr.forEach(sl => {
    ...
    keepSearchList() {
      ...
      if (this.tableAttr.tableSearchKey) {
        ...
        let obj =
          JSON.parse(sessionStorage.ts_list) === '{}'
            ? {}
            : JSON.parse(sessionStorage.ts_list)
    ...
    onSelectAll(selection) {
      // 单选模式不让选择全选
      if (
        this.tableAttr.isSingleSelect &&
        this.baseData.dataList.length === selection.length &&
        this.baseData.dataList.length > 1
      ) {
